...
class ExtendedTripletMapper(TripletMapper):
    """Extended TripletMapper that also extracts verb and target IDs"""
...
def cal_ivtd(args):
    """
    Calculate IVT metrics from saved prediction txt files.
    Independent mode that doesn't run YOLO model.
    """
    # Get prediction directory
    if args.cal_ivtd:
        prediction_dir = args.cal_ivtd
    else:
        # Default: find most recent test directory
    ...
    for pred_file in pred_files:
        ...
        # Extract video name (before last underscore)
    ...
    for video_idx, (video_name, filenames) in enumerate(video_groups.items()):
        # print(f"Video {video_idx + 1}/{len(video_groups)}: {video_name}")
        ...
        # print(f"GT files: {gt_found}/{len(filenames)}, Pred files: {pred_found}/{len(filenames)}")
        # print(f"Total targets: {sum(len(t) for t in video_targets)}")
        # print(f"Total predictions: {sum(len(p) for p in video_predictions)}")
    ...
    print("=" * 80)
    print(f"\n{'Metric Type':<12} {'Component':<8} {'mAP@50':<8} {'mAP@50-95':<10} {'Precision':<10} {'Recall':<8} {'F1':<8} {'AR@50-95':<10}")
    print("-" * 80)
...
def main():
    ...
    # IVT metrics calculation mode (independent)
